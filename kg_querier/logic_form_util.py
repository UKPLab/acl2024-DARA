...
def lisp_to_sparql(lisp_program: str, get_relation=False, get_type=False):
    clauses = []
    order_clauses = []
            ...
            if isinstance(subp[1], list):  # R relation
                if subp[2][:2] in ["m.", "g."]:  # entity
                    ...
                    if subp[2].__contains__('^^'):
                        ...
                        if data_type in ['integer', 'float', 'dateTime', 'double']:
                ...
                if subp[2][:2] in ["m.", "g."]:  # entity
                    ...
                    if subp[2].__contains__('^^'):
                        ...
                        subp[2] = " ".join(subp[2].split('_'))
                        clauses.append("?x" + i + " ns:" + subp[1] + " " + "?sk0 .")
                        clauses.append(f"FILTER (str(?sk0) = '{subp[2]}')")
    ...
    if get_relation:
        clauses.insert(0, f"?x ?outgoing ?obj .")
        clauses.insert(0, "WHERE {")
        clauses.insert(0, f"SELECT DISTINCT ?outgoing")
    
    elif get_type:
        clauses.insert(0, f"?x ns:type.object.type ?type .")
        clauses.insert(0, "WHERE {")
        clauses.insert(0, f"SELECT DISTINCT ?type")
...
if __name__ == '__main__':
    a = lisp_to_sparql('(ARGMIN medicine.medical_trial_type (R medicine.medical_trial.type_of_trial) medicine.medical_trial.end_date)')
    print(a)