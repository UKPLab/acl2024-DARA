...
sparql = SPARQLWrapper("https://square.ukp-lab.de/virtuoso/sparql/")
sparql.setReturnFormat(JSON)
current_dir = os.path.dirname(os.path.realpath(__file__))

with open(os.path.join(current_dir, 'kg_data/ontology/fb_roles'), 'r') as f:
...
def execute_query(query: str) -> List[str]:
    ...
    for result in results['results']['bindings']:
        for var in result:
            if var not in var_dic:
                var_dic[var] = []
            var_dic[var].append(result[var]['value'].replace('http://rdf.freebase.com/ns/', '').replace("-08:00", ''))
    return var_dic
...
def get_in_relations(entity: str):
    ...
    for result in results['results']['bindings']:
        in_relations.append(result['value']['value'].replace('http://rdf.freebase.com/ns/', ''))

    return in_relations

def get_desc_of_relation(relation: str):
    ...
    """ + "?x :type.object.id " + f"'{relation}' .\n" + " ?x :common.topic.description ?value .}" 
    sparql.setQuery(query)
    try:
        results = sparql.query().convert()
    except urllib.error.URLError:
    ...
    if results['results']['bindings']:
        result = results['results']['bindings'][0]['value']['value']
    else:
        result = ''
    return result
...
def get_out_relations(entity: str):
    ...
    #     exit(0)
    for result in results['results']['bindings']:
        out_relations.append(result['value']['value'].replace('http://rdf.freebase.com/ns/', ''))
...
def get_2hop_relations(entity: str):
    ...
    for result in results['results']['bindings']:
        r1 = result['r1']['value'].replace('http://rdf.freebase.com/ns/', '')
        r0 = result['r0']['value'].replace('http://rdf.freebase.com/ns/', '')
        # depends on direction of rlations
